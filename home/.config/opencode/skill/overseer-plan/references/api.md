...
interface Task {
  ...
  blockedBy: string[];
  blocks: string[];
  // VCS tracking (populated by start/complete)
  bookmark?: string;      // VCS bookmark name (e.g., "task/task_01ABC")
  startCommit?: string;   // Commit ID when task started
  commitSha?: string;     // Commit ID when task completed
}
```
